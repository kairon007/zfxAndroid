package com.zifei.corebeau.extra.qiniu.config;
...
public class Config {
	...
	/**
	 * HTTP连接超时的时间毫秒(ms) Determines the timeout in milliseconds until a
	 * connection is established. A timeout value of zero is interpreted as an
	 * infinite timeout.
	 * 
	 * Please note this parameter can only be applied to connections that are
	 * bound to a particular local address.
	 */
	public static int CONNECTION_TIMEOUT = 60 * 1000;
	/**
	 * 读取response超时的时间毫秒(ms) Defines the socket timeout (<code>SO_TIMEOUT</code>
	 * ) in milliseconds, which is the timeout for waiting for data or, put
	 ...
	 */
	public static int SO_TIMEOUT = 60 * 1000;
	...
	public static int ONCE_WRITE_SIZE = 1024 * 8;
